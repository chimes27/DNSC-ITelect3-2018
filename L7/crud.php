...
class CRUD{
	...
	function searchRecord($searchTxt){
		$sql = "SELECT * FROM personInfo WHERE fname='$searchTxt' OR lname='$searchTxt' OR email='$searchTxt'";
		$data = $this->con->query($sql);
		if($data->num_rows == 0){
			echo "The table is empty";
		}else{
		...
		while($row = $data->fetch_assoc()){
			...
			echo "<td>". $row['gender'] . "</td>";
			echo "<td>". $row['email'] . "</td>";
			echo "<td><a href='http://localhost/itElect3/lessons/l7/edit.php?id=". $row['id'] . "'>Edit</a></td>";
			echo "</tr>";
		}
		echo "</table>";
	}
